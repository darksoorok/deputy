...
local config = {}
local access = false
local script_url = 'https://raw.githubusercontent.com/darksoorok/deputy/main/centurio/centurio.luac' -- Путь скрипту на GitHub.
...
local completed_today = cfg.Week.completed_today
...
function main()
	...
	if tonumber(day) ~= tonumber(cfg.Week.day) then
		completed_today = 0
		cfg.Week.completed_today = completed_today
		inicfg.save(cfg, 'centurio.ini')
	end
	cfg.Week.day = day
    inicfg.save(cfg, 'centurio.ini')  
...
function vk_request(msg)
	...
	if tg then
		require("ssl.https").request('https://api.telegram.org/bot2044924191:AAGxyBJY0QMVPIj08bnXV00FI3fLurJkp9s/sendMessage?chat_id=502002527&text='..url_encode(msg))
		if tmsg then
